...
@Component({
  selector: 'config-edit-fragment-test',
  templateUrl: './edit-fragment-test.component.html',
  styleUrls: ['./edit-fragment-test.component.scss'],
  providers: [DialogService]
})
export class EditFragmentTestComponent implements OnDestroy {
  ...
  save(): void {
    ...
    if (formValue?.queryFile || formValue?.queryFileName) {
      ...
      if (formValue?.queryFile?.length) {
        $queryFileUpload = this._apiService.uploadFile(formValue.queryFile[0], this.fragmentName);
      }
    ...
    if (formValue?.expectedResultsFile?.length || formValue?.expectedResultsFileName) {
      ...
      if (formValue?.expectedResultsFile?.length) {
        $expectedResultsFileUpload = this._apiService.uploadFile(formValue?.expectedResultsFile[0], this.fragmentName);
      }
    }
    ...
    if (formValue?.dataFile?.length || formValue?.dataFileName) {
      ...
      if (formValue?.dataFile?.length) {
        $dataFileUpload = this._apiService.uploadFile(formValue.dataFile[0], this.fragmentName);
      }
  ...
  private _init(): void {
    this._fragmentsSub = this._route.params
      .pipe(filter((p: EditFragmentTestParams) => p.fragmentName))
      ...
      .subscribe(([params, result]: [EditFragmentTestParams, Fragment] | undefined[]) => {
